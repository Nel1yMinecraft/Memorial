...
import me.memorial.utils.GLUtils
            ...
            "2dtag" -> {
                for (o in mc.theWorld.playerEntities) {
                    ...
                    if (e.isEntityAlive && e !== mc.theWorld.playerEntities) {
                        ...
                        val pX: Double =
                            e.lastTickPosX + (e.posX - e.lastTickPosX) * mc.timer.renderPartialTicks - renderManager.renderPosX
                        val pY: Double =
                            e.lastTickPosY + (e.posY - e.lastTickPosY) * mc.timer.renderPartialTicks - renderManager.renderPosY
                        val pZ: Double =
                            e.lastTickPosZ + (e.posZ - e.lastTickPosZ) * mc.timer.renderPartialTicks - renderManager.renderPosZ
    ...
    private fun renderNameTag(entity: EntityLivingBase, tag: String, pX: Double, pY: Double, pZ: Double) {
        when (typeValue.get().lowercase(Locale.getDefault())) {
            "2dtag" -> {
                ...
                val distanceText =
                    if (distanceValue.get()) "§a[§f" + mc.thePlayer.getDistanceToEntity(entity).toInt() + "§a] " else ""
                ...
                mc.fontRendererObj.drawString(
                    USERNAME,
                    (-STRING_WIDTH).toFloat(),
                    (fr.FONT_HEIGHT - 22).toFloat(),
                    16777215,
                    fontShadowValue.get()
                )
        ...
        when (typeValue.get().lowercase(Locale.getDefault())) {
            "2dtag" -> {
                ...
                Minecraft.getMinecraft().getRenderItem()
                    .renderItemOverlays(Minecraft.getMinecraft().fontRendererObj, stack, x, y)
