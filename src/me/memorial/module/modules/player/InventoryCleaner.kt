...
import me.memorial.events.impl.*
...
@ModuleInfo(name = "InvManager",  description = "Automatically throws away useless items, and also equips armors for you.", category = ModuleCategory.PLAYER)
class InventoryCleaner : Module() {
    ...
    private val invSpoofOld = BoolValue("InvSpoof-Old", false)
    ...
    fun onMotion(event: MotionEvent) {
        if (eventModeValue.get().equals("motion${event.state.stateName}", true))
            performManager()
    }

    @EventTarget(ignoreCondition = true)
    fun onOpen(event: ScreenEvent) {
        if (delayOnStart.get()) InventoryUtils2.CLICK_TIMER.reset()
    }

    fun performManager() {
        if ((noScaffoldValue.get() && (Memorial.moduleManager.getModule(Scaffold::class.java)!!.state))|| !InventoryUtils2.CLICK_TIMER.hasTimePassed(delay) ||
        ...
        if (cleanGarbageValue.get()) while (InventoryUtils2.CLICK_TIMER.hasTimePassed(delay)) {
            ...
            if (delay == 0L || InventoryUtils2.CLICK_TIMER.hasTimePassed(delay))
