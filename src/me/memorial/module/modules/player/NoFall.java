...
import me.memorial.utils.VecRotation;
...
public class NoFall extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (!getState() || Memorial.moduleManager.getModule(FreeCam.class).getState())
            return;
    ...
    private void onMotionUpdate(MotionEvent event) {
        ...
        if (event.getEventState() == EventState.PRE) {
            ...
            if (mc.thePlayer.fallDistance > minFallDistance.get()) {
                ...
                boolean ok = new Vec3(mc.thePlayer.posX, mc.thePlayer.posY + mc.thePlayer.getEyeHeight(), mc.thePlayer.posZ).distanceTo(new Vec3(collision.getPos()).addVector(0.5, 0.5, 0.5)) < mc.playerController.getBlockReachDistance() + Math.sqrt(0.75);
