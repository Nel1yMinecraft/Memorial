...
import me.memorial.utils.TimerUtils
...
class ChestStealer : Module() {
    ...
    fun performStealer(screen: GuiScreen) {
        ...
        // inventory cleaner
        val inventoryCleaner = Memorial.moduleManager.getModule(InventoryCleaner::class.java) as InventoryCleaner
        ...
        if (!isEmpty(screen) && !(closeOnFullValue.get() && fullInventory)) {
            ...
            if (silenceValue.get() && !stillDisplayValue.get()) ClientUtils.displayChatMessage("ChestStealer || Closed chest.")
