...
public class AntiBot extends Module {
    ...
    public static boolean isBot(final EntityLivingBase entity) {
        ...
        final AntiBot antiBot = (AntiBot) Memorial.moduleManager.getModule(AntiBot.class);
        ...
        if(antiBot.duplicateInWorldValue.get()) {
            if (mc.theWorld.loadedEntityList.stream()
                    .filter(currEntity -> currEntity instanceof EntityPlayer && currEntity
                            .getDisplayName().getUnformattedText().equals(currEntity.getDisplayName().getUnformattedText()))
                    .count() > 1)
                return true;
        }
