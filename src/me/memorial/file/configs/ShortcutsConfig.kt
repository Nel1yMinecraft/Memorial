...
import me.memorial.command.shortcuts.Shortcut
...
class ShortcutsConfig(file: File) : FileConfig(file) {
    ...
    override fun loadConfig() {
        ...
        for (shortcutJson in jsonElement) {
            ...
            for (scriptCommand in scriptJson) {
                ...
                val command = Memorial.commandManager.getCommand(commandName) ?: continue

                script.add(Pair(command, arguments.map { it.asString }.toTypedArray()))
            }

            Memorial.commandManager.registerCommand(Shortcut(name, script))
        }
    }
    ...
    override fun saveConfig() {
        ...
        for (command in Memorial.commandManager.commands) {
            ...
            for (pair in command.script) {
                ...
                for (argument in pair.second) {
                    argumentsObject.add(JsonPrimitive(argument))
                }
