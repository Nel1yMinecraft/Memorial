...
import me.memorial.module.modules.render.ColorMixer
...
import me.memorial.ui.client.hud.element.Side.Horizontal
import me.memorial.ui.client.hud.element.Side.Vertical
import me.memorial.ui.font.AWTFontRenderer
import me.memorial.ui.font.Fonts
import me.memorial.utils.render.BlurUtils
import me.memorial.utils.render.ColorUtils
import me.memorial.utils.render.RenderUtils
import me.memorial.utils.render.ShadowUtils
import me.memorial.value.*

import me.memorial.utils.AnimationUtils
...
class Arraylist(x: Double = 1.0, y: Double = 2.0, scale: Float = 1F,
            ...
            if (module.array && (module.state || module.slide != 0F)) {
                ...
                when (hAnimation.get()) {
                    ...
                    "Default" -> {
                        if (module.state) {
                            if (module.slide < width) {
                                module.slide = me.memorial.utils.render.AnimationUtils.easeOut(module.slideStep, width.toFloat()) * width
        ...
        when (side.horizontal) {
            Horizontal.RIGHT, Horizontal.MIDDLE -> {
                            ...
                            RenderUtils.newDrawRect(
                                    ...
                                    when (shadowColorMode.get().lowercase(Locale.getDefault())) {
                                        ...
                                        "text" -> {
                                            ...
                                            val Sky = RenderUtils.SkyRainbow(
                                                    counter[0] * (skyDistanceValue.get() * 50),
                                                    saturationValue.get(),
                                                    brightnessValue.get()
                                            )
                                            val CRainbow = RenderUtils.getRainbowOpaque(
                                            ...
                                            val Nostalgia = module.category.getColor()
                ...
                modules.forEachIndexed { index, module ->
                    ...
                    val Sky: Int = RenderUtils.SkyRainbow(counter[0] * (skyDistanceValue.get() * 50), saturationValue.get(), brightnessValue.get())
    ...
    override fun updateElement() {
        modules = if (abcOrder.get()) Memorial.moduleManager.modules
                .filter { it.array && (if (hAnimation.get().equals("none", ignoreCase = true)) it.state else it.slide > 0) }
        else Memorial.moduleManager.modules
                .filter { it.array && (if (hAnimation.get().equals("none", ignoreCase = true)) it.state else it.slide > 0) }
                .sortedBy { -fontValue.get().getStringWidth(getModName(it)) }
        sortedModules = if (abcOrder.get()) Memorial.moduleManager.modules.toList()
        else Memorial.moduleManager.modules.sortedBy { -fontValue.get().getStringWidth(getModName(it)) }.toList()
    }
    ...
    fun getModName(mod: Module): String {
        var displayName: String = (mod.name) + getModTag(mod)
