package me.memorial.ui.client.altmanager;

import com.google.gson.JsonElement;
import com.google.gson.JsonObject;
import com.google.gson.JsonParser;
import me.memorial.Memorial;
import me.memorial.ui.client.altmanager.sub.altgenerator.GuiMCLeaks;
import me.memorial.ui.client.altmanager.sub.*;
import me.memorial.ui.font.Fonts;
import me.memorial.utils.ClientUtils;
import me.memorial.utils.login.LoginUtils;
import me.memorial.utils.login.MinecraftAccount;
import me.memorial.utils.login.UserUtils;
...
public class GuiAltManager extends GuiScreen {
    ...
    public static void loadGenerators() {
        try {
            ...
            final JsonElement jsonElement = new JsonParser().parse(HttpUtils.get(Memorial.CLIENT_CLOUD + "/generators.json"));
    ...
    public static String login(final MinecraftAccount minecraftAccount) {
        ...
        if (result == LoginUtils.LoginResult.LOGGED) {
            ...
            Memorial.fileManager.saveConfig(Memorial.fileManager.accountsConfig);
    ...
    public void initGui() {
        ...
        for (int i = 0; i < Memorial.fileManager.accountsConfig.getAccounts().size(); i++) {
            MinecraftAccount minecraftAccount = Memorial.fileManager.accountsConfig.getAccounts().get(i);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.font35.drawCenteredString(this.searchField.getText().isEmpty() ? (Memorial.fileManager.accountsConfig.getAccounts().size() + " Alts") : this.altsList.accounts.size() + " Search Results", width / 2.0f, 18, 0xffffff);
        ...
        switch (button.id) {
            ...
            case 2:
                if (altsList.getSelectedSlot() != -1 && altsList.getSelectedSlot() < altsList.getSize()) {
                    Memorial.fileManager.accountsConfig.removeAccount(this.altsList.accounts.get(altsList.getSelectedSlot()));
                    Memorial.fileManager.saveConfig(Memorial.fileManager.accountsConfig);
            ...
            case 7:
                ...
                while ((line = bufferedReader.readLine()) != null) {
                    ...
                    if (!Memorial.fileManager.accountsConfig.accountExists(accountData[0])) {
                        if (accountData.length > 1)
                            Memorial.fileManager.accountsConfig.addAccount(accountData[0], accountData[1]);
                        else
                            Memorial.fileManager.accountsConfig.addAccount(accountData[0]);
                    }
                }
            ...
            case 11:
                ...
                break;
            case 12:
                if (Memorial.fileManager.accountsConfig.getAccounts().size() == 0) {
                    status = "§cThe list is empty.";
                    return;
                }
                ...
                try {
                    ...
                    for (MinecraftAccount account : Memorial.fileManager.accountsConfig.getAccounts()) {
    ...
    private class GuiList extends GuiSlot {
        ...
        private void updateAccounts(String search) {
            if (search == null || search.isEmpty()) {
                this.accounts = Memorial.fileManager.accountsConfig.getAccounts();
                return;
            }
